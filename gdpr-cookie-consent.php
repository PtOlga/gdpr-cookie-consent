...
class GDPR_Cookie_Consent {
    ...
    public function render_banner() {
        ...
        $btn_save = esc_html($options['button_save_text'] ?? 'Save');
        ...
        $cat_necessary_desc = esc_html($options['category_necessary_desc'] ?? 'essential');
        $cat_analytics_desc = esc_html($options['category_analytics_desc'] ?? 'analytics');
        $cat_marketing_desc = esc_html($options['category_marketing_desc'] ?? 'advertising');
        ...
        <div id="gdpr-cookie-banner" class="gdpr-banner" style="background-color: <?php echo $banner_bg; ?>; color: <?php echo $banner_text; ?>;">
            <div class="gdpr-banner-content">
                <div class="gdpr-banner-text">
                    <?php echo $message; ?>
                    <span class="gdpr-banner-categories-info">(1.<?php echo $cat_necessary_desc; ?>, 2.<?php echo $cat_analytics_desc; ?>, 3.<?php echo $cat_marketing_desc; ?>)</span>
                    <?php if ($privacy_url): ?>
                        <a href="<?php echo $privacy_url; ?>" target="_blank" style="color: <?php echo $banner_text; ?>;"><?php echo $privacy_text; ?></a>
                    <?php endif; ?>
                </div>
                
                <div class="gdpr-banner-controls">
                    <div class="gdpr-categories">
                        <label class="gdpr-category" title="<?php echo $cat_necessary_desc; ?>">
                            <input type="checkbox" checked disabled data-category="necessary">
                            <span><?php echo $cat_necessary; ?></span>
                        </label>
                        <label class="gdpr-category" title="<?php echo $cat_analytics_desc; ?>">
                            <input type="checkbox" data-category="analytics">
                            <span><?php echo $cat_analytics; ?></span>
                        </label>
                        <label class="gdpr-category" title="<?php echo $cat_marketing_desc; ?>">
                            <input type="checkbox" data-category="marketing">
                            <span><?php echo $cat_marketing; ?></span>
                        </label>
                    </div>
    ...
    public function register_settings() {
        ...
        add_settings_field('button_save_text', __('Save Button', 'gdpr-cookie-consent'), 
            [$this, 'render_text_field'], 'gdpr-cookie-consent', 'gdpr_cc_text', 
            ['field' => 'button_save_text', 'default' => 'Save']);
        ...
        add_settings_field('category_necessary_desc', __('Necessary - Description', 'gdpr-cookie-consent'), 
            [$this, 'render_text_field'], 'gdpr-cookie-consent', 'gdpr_cc_categories', 
            ['field' => 'category_necessary_desc', 'default' => 'essential']);
        ...
        add_settings_field('category_analytics_desc', __('Analytics - Description', 'gdpr-cookie-consent'), 
            [$this, 'render_text_field'], 'gdpr-cookie-consent', 'gdpr_cc_categories', 
            ['field' => 'category_analytics_desc', 'default' => 'statistics']);
        ...
        add_settings_field('category_marketing_desc', __('Marketing - Description', 'gdpr-cookie-consent'), 
            [$this, 'render_text_field'], 'gdpr-cookie-consent', 'gdpr_cc_categories', 
            ['field' => 'category_marketing_desc', 'default' => 'advertising']);
    ...
    public function sanitize_options($input) {
        ...
        $sanitized['button_save_text'] = sanitize_text_field($input['button_save_text'] ?? 'Save');
        ...
        $sanitized['category_necessary_desc'] = sanitize_text_field($input['category_necessary_desc'] ?? 'essential');
        $sanitized['category_analytics_label'] = sanitize_text_field($input['category_analytics_label'] ?? 'Analytics');
        $sanitized['category_analytics_desc'] = sanitize_text_field($input['category_analytics_desc'] ?? 'statistics');
        $sanitized['category_marketing_label'] = sanitize_text_field($input['category_marketing_label'] ?? 'Marketing');
        $sanitized['category_marketing_desc'] = sanitize_text_field($input['category_marketing_desc'] ?? 'advertising');
    ...
    public function render_settings_page() {
        ...
        <div class="wrap">
            ...
            <div style="background: #fff; padding: 20px; margin: 20px 0; border-left: 4px solid #22c55e;">
                ...
                <div id="gdpr-preview" style="margin-top: 15px; padding: 10px 20px; border-radius: 4px; font-size: 13px; background: <?php echo esc_attr($options['banner_bg_color']); ?>; color: <?php echo esc_attr($options['banner_text_color']); ?>;">
                    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 15px;">
                        <div style="font-size: 12px;">
                            <?php echo esc_html($options['banner_message']); ?>
                            <span style="opacity: 0.75; font-size: 11px; margin: 0 5px;">(1.<?php echo esc_html($options['category_necessary_desc']); ?>, 2.<?php echo esc_html($options['category_analytics_desc']); ?>, 3.<?php echo esc_html($options['category_marketing_desc']); ?>)</span>
                        ...
                        <div style="display: flex; align-items: center; gap: 15px;">
                            <div style="display: flex; align-items: center; gap: 12px; font-size: 12px;">
                                <label style="display: flex; align-items: center; gap: 4px; opacity: 0.6;">
                                    ...
                                    <span style="font-weight: 500;"><?php echo esc_html($options['category_necessary_label']); ?></span>
                                </label>
                                <label style="display: flex; align-items: center; gap: 4px; cursor: pointer;">
                                    <input type="checkbox" style="width: 14px; height: 14px;">
                                    <span style="font-weight: 500;"><?php echo esc_html($options['category_analytics_label']); ?></span>
                                </label>
                                <label style="display: flex; align-items: center; gap: 4px; cursor: pointer;">
                                    <input type="checkbox" style="width: 14px; height: 14px;">
                                    <span style="font-weight: 500;"><?php echo esc_html($options['category_marketing_label']); ?></span>
                                </label>
                            </div>
                            <div style="display: flex; gap: 8px;">
                                <button type="button" style="padding: 5px 10px; border-radius: 4px; font-size: 12px; cursor: default; background: transparent; border: 1px solid <?php echo esc_attr($options['banner_text_color']); ?>; color: <?php echo esc_attr($options['banner_text_color']); ?>; opacity: 0.7;"><?php echo esc_html($options['button_reject_text']); ?></button>
                                <button type="button" style="padding: 5px 10px; border-radius: 4px; font-size: 12px; cursor: default; background: transparent; border: 1px solid <?php echo esc_attr($options['banner_text_color']); ?>; color: <?php echo esc_attr($options['banner_text_color']); ?>; opacity: 0.7;"><?php echo esc_html($options['button_save_text']); ?></button>
                                <button type="button" style="padding: 6px 12px; border-radius: 4px; font-size: 12px; cursor: default; border: none; background: <?php echo esc_attr($options['button_bg_color']); ?>; color: <?php echo esc_attr($options['button_text_color']); ?>;"><?php echo esc_html($options['button_accept_text']); ?></button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    private function get_all_options() {
        $defaults = [
            ...
            'category_necessary_desc' => 'essential',
            'category_analytics_label' => 'Analytics',
            'category_analytics_desc' => 'statistics',
            'category_marketing_label' => 'Marketing',
            'category_marketing_desc' => 'advertising',
        ];
